...
WebView 내부에서는 Three.js 기반의 3D 씬을 구성하여, 가방과 아이템을 실제 비율로 시각화합니다.
가방은 고정된 위치에 박스 형태로 렌더링되며, 아이템은 위치, 크기, 회전 값에 따라 배치됩니다.
사용자의 화면 해상도와 디바이스 비율에 따라 카메라 시점과 조명 위치가 자동 조정되어 모든 요소가 중앙에 안정적으로 표시되도록 구성했습니다.

## 상호작용 및 컨트롤
사용자는 시뮬레이터 내에서 아이템을 **드래그하여 이동**, **선택 후 삭제**, **회전** 등의 인터랙션을 수행할 수 있습니다.
이러한 상호작용은 `OrbitControls`, `Raycaster` 등을 활용하여 구현되며, 삭제는 **휴지통 영역과의 교차 감지**로 처리됩니다.
선택된 아이템은 색상 변경 및 외곽선 강조를 통해 시각적으로 구분됩니다.

## 양방향 통신 구조
React Native 앱과 WebView는 `postMessage` / `onMessage` 기반의 메시지 통신 구조를 사용합니다.
- 앱 → WebView: 아이템 추가, 크기 수정, 초기 렌더링 명령 전송
- WebView → 앱: 아이템 이동, 선택, 삭제 등 사용자 조작 결과 전달
이 구조를 통해 앱과 시뮬레이터 간 상태 동기화가 실시간으로 이루어집니다.

## 렌더링 최적화
초기 로딩 시 WebView는 앱에 `WEBVIEW_READY` 메시지를 전달하며,
앱은 해당 신호를 받은 이후에만 렌더링 데이터를 전달합니다.
이를 통해 **중복 렌더링 방지 및 초기 화면 안정화**가 가능하며,
화면 회전 및 디바이스 리사이즈에도 대응할 수 있도록 **카메라 및 캔버스 리사이징 로직**을 포함하고 있습니다.

## 구조 확장성
시뮬레이터는 React + Three.js 기반으로 구성되어 있어 다음과 같은 확장이 용이합니다
- 복수 가방 렌더링
- 사용자 정의 색상 및 테마
- 시뮬레이션 애니메이션 추가
- ARKit 등 네이티브 AR 기능 연동
또한, 모든 렌더링 로직과 메시지 핸들러는 역할 단위로 분리되어 있어 **별도 웹 앱으로의 재사용도 가능**합니다.
...
| 기술 스택 | 설명 |
|-----------|------|
| **React** | WebView 내부의 UI 및 상태 관리를 위해 사용된 프레임워크입니다. Three.js와 함께 구성 요소를 컴포넌트 단위로 모듈화하여 구조적 개발이 가능합니다. |
| **Three.js** | 3D 씬, 가방 및 아이템 렌더링, 카메라/조명 제어 등 전체 시뮬레이션 시각화를 담당하는 핵심 라이브러리입니다. 박스 및 메쉬 오브젝트, OrbitControls 등을 활용합니다. |
...
### 문제: React Native는 3D 시뮬레이션에 적합하지 않다
...
### 결과: RN 앱에서도 실시간 3D 시뮬레이션이 가능해졌다
